...
ForgePilot 以 Git Commit / PR 的 Diff 为输入，结合项目自己的知识库和大模型，产出结构化审查报告：
**风险等级、问题定位、证据来源、修复建议**。
...
**已完成**：r1–r6、P5、P6、P7（工作台、研发度量、研发助手、Ink 前端收尾）以及生产化加固
（TLS、Prometheus 告警、审查反馈闭环、前端错误上报、数据生命周期）均已归档；P7 已完成后端/前端
focused tests、前端构建和本地质量门。

**进行中**：

- **P8 实验、答辩与功能冻结** —— 38 例 manifest 已升级到 v2，全部补齐 Requirement/AC/consistency truth；
  Baseline/A/B/C/D runner、AC 判分、运行 metadata、ForgePilot 展示层和真实 HMAC 离线注入脚本已落地。
  真实模型五臂矩阵仍必须在 Docker-enabled 隔离栈上执行，不能用 H2/Mock 数字替代。
- **r7/r8 评测与提示词线** —— 38 例确定性建仓、`d3-v1` 两率、分层模板和生产 feature flags 已落地；
  最终 38 例真实模型复跑、五臂数字与漏报回灌仍以 P8 `eval-runs/` 产物为准。
- **P9 品牌与仓库收尾** —— ForgePilot 用户可见文案已先行切换；远程仓库改名、徽章/链接重定向验证和简历同步
  依赖 P8 证据冻结，并需在执行远程改名前再次确认影响面。
