...
model-service  FastAPI 模型服务(已下线,目录保留仅作历史参考)
...
> 上面两份够跑通链路。要跑 `docs/演示素材与缺陷对照表.md` 第五节的三档对照，得按每个项目 12 份传（该仓库 `docs/` 下 4 份 + `demo-repos/knowledge-shared/` 5 份 + `demo-repos/knowledge-noise/` 3 份）。

6. 查询 commit 列表，能看到 `main` 上的两个提交。**审查目标不要选最新 commit** —— 缺陷全部在 PR 分支上，`main` 的 HEAD（`6977028`）只是知识文档提交，审它得到的 diff 里没有代码；而 commit 列表只列绑定分支的提交，PR 分支的提交不会出现在里面。在审查表单的「Commit ID」里手工填 PR 分支 HEAD `e205e0e`，Base Commit 留空即可（它的父提交正是 `main` 的 HEAD，diff 就是完整的 PR 内容）。
7. 触发审查任务。
8. 查看报告，dev 模式下会自动加载最新报告。**mock 模式下预期只有 1 条 `SQL_INJECTION`**，见下方说明。
9. 对一个 issue 提交反馈，例如 `TRUE_POSITIVE`。
10. 查看 MQ 日志区域，dev inline 模式下也会保留任务处理日志。

> **mock 模式的预期产出就是 1 条，不要当成链路坏了。** dev 默认 `AI_PROVIDER=mock`（见第 10 节），`MockAiReviewClient` 是纯字符串匹配的规则引擎，四条规则对 mall 的这份 PR diff 逐条求值：`AUTH_RISK` 要 diff 里出现 `/admin`（没有）、`NULL_POINTER` 要 `findById` / `getById`（没有）、`BUSINESS_RULE_RISK` 要 `payStatus`（没有），只有 `SQL_INJECTION` 的「`select` + 字符串拼接」特征命中 —— 触发它的是 `PromotionShipService.searchActivityOrders` 里的拼接，对应对照表的 **M5**。
>
> 另有两点别误判：一是这条 issue 的文件名指向 diff 里的**第一个**文件 `PromotionController.java`、且没有行号，那是 mock 的 `guessFile` 直接取首个 `+++ b/` 的结果，不代表定位能力；二是它的证据字段是写死的「静态特征: SQL + 字符串拼接」，**不引用知识库** —— mock 只有 `AUTH_RISK` 与 `BUSINESS_RULE_RISK` 两条规则会带 RAG 证据，而这两条在这份 diff 上都不触发。想看 RAG 起没起作用，用知识库页的「测试检索」，或切到真实模型。
>
> 要看到 `AUTH_RISK`、业务规则风险等覆盖 M1~M10 的输出，必须把 `AI_PROVIDER` 切到真实模型（配置见 `docs/12_服务器部署与演示手册.md` 第 7 节）。mock 只用来验证链路连通。
...
- 是否绑定了 `demo-repos/mall-order-service`，且该仓库已由 `scripts/init-demo-repos.sh` 重建过。
- **审查目标是不是填了 PR 分支 HEAD `e205e0e`。** 选 `main` 上的最新 commit 正是「报告没有 issue」最常见的原因 —— 那次提交只有知识文档，没有代码变更。
...
该命令依次执行后端测试、前端测试和前端生产构建。需要验证完整 HTTP 链路时去掉 `-SkipSmoke`。
