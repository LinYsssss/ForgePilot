...
后端 8 个顶层包、20 张业务表、10 个 Flyway 迁移、332 个测试零跳过；前端 6 个一级导航、11 条产品路由（含非一级 `/account`）、37 个测试。

已交付能力：

- **auth**：本地账号、显示名、进程内会话、Cookie CSRF、会话版本失效、改密。
- **project**：可按显示名/用户名/平台 ID 搜索的成员目录、原子批量添加、成员多角色、唯一 LEADER 与角色能力并集、成员移除。
