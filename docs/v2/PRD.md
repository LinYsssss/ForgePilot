...
状态：**R2.5 顶部导航基线（2026-08-23）**。本文是**产品权威**：定位、角色、范围、流程与验收标准。Phase 0–8 的历史闸门保持有效；D017 的主链路补全与 D018 的导航布局各由一个独立 Trellis 任务执行并已完成。
...
| P11 | "本人 PR" 由项目级 SCM 稳定外部 ID 判定，**禁止按用户名授权**；身份由 LEADER 配置，开发者不得自行声明 | [D010](./DECISIONS.md#d010) |
...
自动化覆盖（后端 316 个测试、前端 35 个测试）证明的部分记 `[x]`；只有人工浏览器验收能证明的部分保持 `[ ]` 并注明，不因为“阶段已通过”就一律打勾。
