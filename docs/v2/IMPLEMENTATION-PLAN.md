# ForgePilot V2 最终候选实施蓝图（仅规划）

权威方案：`final-candidate.md`。当前没有实施授权；本文件所有 Phase 均为 pending。
...
- 人工批准 `final-candidate.md` 后，冻结一句话主线、角色、Requirement/Finding/Review 状态。
- 冻结 14 表、顶层包、allowed dependencies 和 SCM event contract。
- 为 KEEP 白名单记录 Legacy SHA、目标测试和适配点。
- 退出条件：没有产品/范围/权限/兼容阻塞问题；仍不得自动开始 Phase 1。

## Phase 1：最小绿地底座

- 单 Spring Boot 应用、Vue 应用、Postgres pgvector。
- 建 8 个顶层包和 `V1__init.sql` 14 表。
...
- 需求附件先创建 KnowledgeDocument，再保存 RequirementAttachment 关系。
...
- 只支持一个 configured embedding model/dimension。
...
- 唯一 Review Engine 一次返回 AC verdict + Finding。
