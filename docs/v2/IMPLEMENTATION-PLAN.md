...
- 成员项目级 SCM 身份由 LEADER 配置，稳定外部 ID 唯一（历史 Phase 2 形态；当前已由 D020 取代）。
- 界面：登录、项目列表、成员管理。
...
## D018 顶部居中导航与演示数据重置（已实现并验证）

- 只改变已登录 Shell 的空间布局与品牌摆放：横版 Logo 在左、六入口在水平中心、账户操作在右；登录页只保留应用图标。
- 路由、权限、项目查询参数、业务状态源、AI 与向量能力均不变；未新增依赖、路由、一级入口或表。
- 同批执行了一次用户明确授权的部署数据清理：在一个事务内清空 15 张业务表，保留唯一启用账号 `ysainlin`，不删卷、不改 Flyway、不新增重置接口。
- 实施与验收证据见 `.trellis/tasks/archive/2026-08/08-23-topnav-ui-data-reset/result.md`。

## R2.5 交付形态复核（2026-08-23）

以一次完整本地验证为准，不引用历史数字：

| 维度 | 复核结果 |
|---|---|
| 后端顶层包 | 8（`common/auth/project/requirement/scm/knowledge/ai/review`） |
| 业务表 / Flyway 迁移 | 16 / 7（`V1`–`V7`） |
| 后端测试 | `./mvnw -B -ntp verify` → 316 passed / 0 failed / 0 skipped，BUILD SUCCESS |
| 前端一级导航 / 产品路由 | 6 / 10 |
| 前端闸门 | lint、strict typecheck、35 个测试、生产构建全部通过 |
| ArchUnit | 6 条边界规则 + 2 条反重言式测试全绿 |

复核当场收口了两条历史欠账：PRD P1 的 DEVELOPER 半条授权**已实现**（[D016.2](./DECISIONS.md#d016) 执行状态），
向量索引经实测后**决策为不建**（[D019](./DECISIONS.md#d019)）。其余未兑现项在
[README §已知缺口](./README.md) 单列，均为明确接受的 MVP 缺口或人工验收项。
...
## D020 成员目录、多角色与 SCM 多身份（2026-08-24）

本次按独立 Trellis 任务执行，保持 8 个后端顶层包、6 个一级入口、单一 Review Engine 与正式评测证据不变。实施顺序固定为：

1. V8 回填显示名，迁移旧角色和旧手填 SCM 身份，再删除 `project_member` 旧列；业务表变为 19 张、Flyway 变为 8 个。
2. 账户响应加入显示名；成员 API 切换为候选搜索、原子批量添加、角色集合和独立 Leader 转移。
3. SCM 增加用户身份验证、标签/用途、项目绑定、默认自动激活与严格审批；按活动绑定重算 PR 作者成员映射。
4. 前端增加非一级 `/account` 页面，成员页完成目录选择、多角色编辑、本人绑定与 Leader 审批；六个一级导航不变。
5. 只补身份 Provider 路径、默认/严格绑定、Token 脱敏和角色能力并集的核心测试；不建立角色×Provider×状态的组合矩阵，不运行负载或正式评测。
6. 完整验证后按用户授权清空当前部署业务数据、重新构建部署；独立网页全链路清单已提供，真实浏览器验收按用户要求延期执行。

最终实测形态：8 个后端顶层包、19 张业务表 / 8 个 Flyway 迁移，后端 `verify` 为
317 passed / 0 failed / 0 skipped；前端保持 6 个一级导航 / 11 条产品路由，lint、strict typecheck、
