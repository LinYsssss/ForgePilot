...
**例外记录**：2026-08-19 的契约复审（R2 / R2.1 / R2.2）中，用户明确批准修订 ADR-002、003、006、007、008、009、011 —— 这些修订与新增的 ADR-009/010/011 属同一批决议，另开新号会使同一规则分散在两处，反而违反单一事实源纪律。各文状态行已标注具体修订轮次。除此之外不得回改已接受的 ADR。
...
| [ADR-003](./ADR-003-review-identity.md) | Review Identity：`UNIQUE NULLS NOT DISTINCT (pull_request_id, head_sha, requirement_revision_id)`；Decision Gate 独立为 (pull_request_id, head_sha)；engine/prompt/model 仅审计元数据 | 已接受 |
...
| [ADR-008](./ADR-008-review-context-and-recovery.md) | Review Context & Recovery：保存不可变上下文快照；head 更新与 PENDING 同事务链，reconciliation 只恢复停滞的 PENDING/RUNNING，不补建 Review | 已接受 |
