...
RABBITMQ_PASSWORD=change-me-to-a-long-random-string

JWT_SECRET=change-me-to-long-random-string
TOKEN_ENCRYPT_KEY=change-me-32-bytes

# 沙箱任务签名密钥（backend 与 sandbox-runner 必须一致）
SANDBOX_SIGNING_SECRET=change-me-to-a-long-random-string

# 认证 Cookie 与 CSRF（合流后 CSRF 默认开启，无需显式配置；
# SECURITY_CSRF_ENABLED=false 仅作为显式回退口）
...
>
> **prod 启动校验**：`APP_ENV=prod` 时后端启动期强制校验密钥质量——
> `JWT_SECRET`、`TOKEN_ENCRYPT_KEY`、`DB_PASSWORD`、`RABBITMQ_PASSWORD`、
> `SANDBOX_SIGNING_SECRET` 均须 ≥16 字符且非占位值，`GRAFANA_ADMIN_PASSWORD` 必须显式提供且不得使用已知默认值，`AUTH_COOKIE_SECURE`
> 必须为 true，配置了种子管理员则口令须 ≥12 字符且非弱口令；不满足直接拒绝启动。
