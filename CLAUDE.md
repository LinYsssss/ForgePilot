...
ForgePilot is a requirement-driven AI code review platform: 9 backend packages, 21 tables across 14 Flyway migrations, a 6-entry / 11-route frontend, GitHub and GitLab providers, and a three-arm evaluation whose holdout was consumed exactly once. The latest backend full run plus failed-class recheck and frontend gates are recorded in `docs/deliverables/TEST-REPORT.html`; read counts from execution evidence rather than copying an older document.
