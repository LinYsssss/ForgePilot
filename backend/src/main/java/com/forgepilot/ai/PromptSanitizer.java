 ...
 * 以及在允许裁剪的 Embedding 输入上执行安全裁剪。Chat 的完整 Prompt 由网关在脱敏后
 * 校验预算并显式拒绝超限。本类是纯函数：不写日志、不碰数据库——一份文档不会因为
 * “看起来含 token”而被拒绝，只会去掉那部分后照常发送。
 *
...
public final class PromptSanitizer {
    ...
    public static String sanitize(String untrusted, int maxChars) {
        return truncate(redact(untrusted), maxChars);
    }

    /** Chat callers must account for the complete redacted prompt rather than silently clip code. */
    public static String redact(String untrusted) {
        return SECRETS.matcher(untrusted).replaceAll(MASK);
    }
