...
`E4 B8`) return **422** with the single `ApiError` body, and `count(*) FROM knowledge_document` is
...
| OPEN-5 | **`ai_call_log` ships in Phase 4 but its `(project_id, review_id) -> review(project_id, id)` FK cannot exist until Phase 6.** (a) column now + FK in the Phase 6 migration, (b) move the table to Phase 6, (c) other? | Blocks the V4 migration entirely. D006 requires such a conflict to go back to a decision ("不得静默降级为无测试的 Service 纪律"); D012.3 keeps that loop mandatory |
...
| OPEN-11 | Cosine (`<=>` / `vector_cosine_ops`) or L2 (`<->` / `vector_l2_ops`)? | Index and query must match exactly or the index is dead weight; migrations are append-only. §5 names no operator; `FoundationDatabaseTest` probes `<->` |
...
| OPEN-14 | §4.1's two signatures cannot supply the four ids `ai_call_log` requires. What is the call-context parameter, and does it stay opaque `Long`s so §1.3 holds? | The signature is a frozen §4.1 quote. `INFERENCE`: an `ai`-owned `AiCallContext` of opaque ids mirrors "knowledge 只收不透明 scope id" |
