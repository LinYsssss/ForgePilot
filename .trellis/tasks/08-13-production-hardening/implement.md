...
6. [x] **冗余清理 backend**（每批独立提交）：候选清单（grep 引用分析+Spring 装配排除）→ 逐批删除 → 每批 mvn verify 绿；「已核未删」保留清单落 `research/redundancy-audit.md`（含 LegacyReviewProjectionService/MockAiReviewClient 保留理由）。（实况：**零删除**——358 个 main 源类中，30 个零直接引用全部框架托管，31 个「仅被测试引用」全部为 Spring 按接口/注册表/监听器装配的生产类，真候选 0 个；结论与复现脚本落 `research/redundancy-audit.md`。方法级死代码与重复代码明确划出本轮范围并说明理由）
7. [x] **注释中文化 backend**（按包分 8-10 批，每批独立提交）：252 个无中文文件治理——英文注释重写中文、关键裸奔逻辑补注释、不写废话注释、不动既有中文注释；每批 mvn verify 绿。完成判据：`LC_ALL=C.UTF-8 grep -rlP "\p{Han}"` 复查全含中文或豁免清单成文（纯常量/生成类）。（实况：252 个里**只有 59 个真有英文散文注释**,已全部重写为中文,复测「仍带英文散文注释」= 0;其余 193 个本就一句注释都没有,豁免清单落 `research/comment-exemptions.md`。另补三处关键裸奔逻辑:两个 Normalizer 的 XXE 全关配置、AgentToolRegistry 的四道闸;并订正 AgentOutboxScheduler/AgentSchedulingConfig 中已被证伪的断言。mvn verify 658/0/6)
8. [x] **尾段（时机门：墨境步骤 6/7 已提交后）**：前端错误上报接线（main.js 挂载 sendBeacon 模块）+ frontend 3 个无中文文件注释治理 + sandbox/model-service 按步骤 0 普查结果同标准治理。验证：npm test + build 绿。（实况 2026-08-14：时机门已开（墨境步骤 7 于 `49737f4` 落库）。①错误上报：新增 `shared/telemetry/clientErrorReporter.js`，main.js 于 mount 前挂载，sendBeacon→keepalive fetch 降级；去重+单页封顶 8 条+堆栈本地截断，全程不抛（防 onerror 自激循环）；补 5 条测试。②frontend 无中文文件 3→0，并澄清 `targetDisappeared` 易读反的语义。③sandbox-runner 48 个无中文文件中真有英文注释的 24 个已全部中文化，其中 5 个是 backend 镜像件（`SandboxJob`/`SandboxJobStatus`/`SandboxResult`/`SandboxJobSigner`/`WorkspaceArchiveReference`），译文与 backend 侧逐字一致以保持镜像；另 3 个被统计脚本误判（`"archive://..."` 字面量里的 `//`），实为零注释。④**model-service 普查已精确圈定**：2350 py 的污染源是未被 venv 规则覆盖的 `model-service/.python-packages/`（vendored numpy/scipy/sklearn，git 未跟踪），真实源码只有 3 个文件，其中 `scripts/train_model.py` 无中文已补注释，现 3/3 全覆盖。验证：npm test 66/0、build 5.69s；`python -m py_compile` 通过；sandbox-runner `mvn verify` 75 跑 0 失败，仅 `symlinkEscapingArchiveRootIsRejected` 因 Windows 本机无符号链接特权报环境错，Linux CI 覆盖该用例）
