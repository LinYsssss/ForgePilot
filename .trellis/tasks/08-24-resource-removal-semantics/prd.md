...
- [ ] AC1 可一次上传多个知识文件，逐文件报告结果，单个失败不阻断整批。
- [ ] AC2 知识文档可删除；`knowledge_chunk` 与 embedding 按既定级联处理，无悬空引用。
- [ ] AC3 被引用为需求附件的知识文档，其删除行为与 R1 所定策略一致，且行为可预期而非数据库报错。
- [ ] AC4 已删除文档的内容不再出现在 Guidance 与 Review 的任何检索结果中。
- [ ] AC5 成员可被移除，且移除后其角色、需求指派能力、Finding 指派能力与项目 SCM 绑定全部失效。
- [ ] AC6 移除成员后，`pull_request` 的不可变作者快照（`author_external_user_id`、`author_username`）、`pull_request_requirement_event`、Finding 血缘与审计记录仍完整可读；`author_user_id` 按预设置空不算破坏事实。
- [ ] AC7 已指派给被移除成员的需求与 Finding 按既定策略处理，并留下可追溯记录。
- [ ] AC8 唯一 `LEADER` 无法被移除；必须先完成负责人转移。
- [ ] AC9 被移除成员的平台账户与自有 SCM 身份不受影响。
- [ ] AC10 只有作废需求可进入删除路径，其他状态一律拒绝。
- [ ] AC11 删除需求后 `ai_call_log` 与 `pull_request_requirement_event` 的记录未被销毁，审计仍可追溯。
- [ ] AC12 三类删除的授权在后端强制；非授权主体经直接调用接口亦不可达。
- [ ] AC13 跨项目删除与跨项目枚举被拒绝；数据库约束层面同样拒绝跨项目写入。
- [ ] AC14 重复删除返回明确结果，不产生歧义状态。
- [ ] AC15 三类删除均有留痕，且留痕不随被删除对象消失。
- [ ] AC16 新增迁移仅为追加，V1–V8 未被修改；空库启动与升级路径均通过。
- [ ] AC17 后端 verify 与前端 lint/typecheck/test/build 全绿零 skip。
- [ ] AC18 真实浏览器完成「批量上传知识 → 删除知识 → 移除成员 → 删除作废需求」人工闭环，且期间既有审计可读。
- [ ] AC19 相关 v2 文档同步更新；正式评测资产未被触碰。
