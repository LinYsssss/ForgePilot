...
- [x] AC1 首次进入 `/workspace` 即有项目上下文；单项目、多项目、无项目三种情况表现均符合 R1 且无空白页。
- [x] AC2 手动切换项目后默认规则不再覆盖用户选择。
- [x] AC3 可一次搜索、多选并批量添加成员且批量套用角色，可逐行覆盖；已是成员的候选不可选。
- [x] AC4 批量提交越界或非法时前端阻止或准确定位到行，且不产生部分成员（后端事务语义未被绕过）。
- [x] AC5 批量路径无法授予或转移 `LEADER`。
- [x] AC6 成员列表为紧凑形态并可按名称与角色筛选，信息项无丢失。
- [ ] AC7 超长质量检查与实现建议输出不撑破布局、不产生页面横向滚动，1440/768/390 人工确认通过。（CSS 已落地并通过 lint/typecheck/build；**三档浏览器确认待执行**，见 `result.md`）
- [ ] AC8 改密可在 `/account` 完成并成功改密；账户菜单点击外部与 `Esc` 均关闭，键盘可达。（改密迁移与「弹层内无口令输入」已自动化；**外部点击/`Esc`/回焦待浏览器确认**，见 `result.md`）
- [x] AC9 两处 Token 输入均有 Provider 获取入口与最小权限说明，且不违反出站 URL 策略。
- [x] AC10 前端 lint/typecheck/test/build 全绿零 skip；6 导航 / 11 路由 + 非一级 account 页形态不变。
- [x] AC11 无后端改动、无迁移、无新依赖；正式评测资产未被触碰。
