...
- [x] AC1 模型输出契约包含问题说明与修复建议，两处 schema 均已同步，且带长度上限。
- [x] AC2 `evidence` 仍是逐字引用；说明/建议与证据在契约与展示上都可区分。
- [x] AC3 校验器对新字段缺失/超限的行为明确且有测试覆盖，无静默丢弃。
- [x] AC4 新迁移仅为追加，V1–V8 未被修改；既有 8 条历史 Finding 在升级后仍合法可读。
- [x] AC5 新字段不在 `findingKey` / `evidenceHash` / `basisHash` 任一输入中；`RULE_VERSION` 仍为 `"1"`。
- [x] AC6 有测试证明模型措辞变化不改变三个哈希，且跨轮次去重、抑制与血缘行为不变。
- [x] AC7 Finding 读取接口暴露新字段，前端按「说明 → 证据 → 建议」层次展示。
- [x] AC8 内部哈希不再作为主要阅读内容；可核验性仍可查看。
- [x] AC9 超长说明与建议不撑破 Review 页布局，不产生页面横向滚动。
- [x] AC10 页面能让使用者区分模型判断与可核验证据，建议不被呈现为已验证结论。
- [x] AC11 置信度的取舍已落入决策记录；若记录则不进哈希、不参与任何自动门禁与状态流转。
- [x] AC12 AI 未获得改变业务状态或自动改码的能力；建议只是建议。
- [x] AC13 新增 D021 起决策记录，且 PRD/ARCHITECTURE/API/DECISIONS 同步更新。
- [x] AC14 后端 verify 与前端 lint/typecheck/test/build 全绿零 skip；空库启动与升级路径通过。
- [x] AC15 **由用户于 2026-08-24 验收通过**（用户判定，非自动化证据；本会话未执行真实 PR 重审，见 `result.md`）。
- [x] AC16 正式评测冻结、语料、holdout 台账与原始输出未被触碰或重跑。
- [x] AC17 `ReviewPrompts.VERSION` 已升为 `"review-2"`，且 `FindingKeys.RULE_VERSION` 仍为 `"1"`；升版未改变任何既有 Finding 的三个哈希。
- [x] AC18 词表外的 `category` / `confidence` 不会中止整批插入：越界值存 `null` 并留 warning，而 `findingKey` 仍使用模型给的原始字符串、逐字节不变。
...
5. **AC15：~~重跑一次真实 PR 审查~~ → 改由用户直接验收。** 原计划重新触发一次真实审查并把输出留档到本任务 `evidence/`。实际执行时该步需要 SCM 与模型凭据且会消耗额度，**用户于 2026-08-24 直接判定验收通过**，故不再重跑，`evidence/` 也不产生。记录口径见 `result.md`：这是用户的验收判定，不是自动化证据。正式评测的冻结、语料清单、holdout 台账与原始输出全程未触碰、未重跑。
