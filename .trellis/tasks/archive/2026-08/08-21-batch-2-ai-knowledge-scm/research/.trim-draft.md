...
**always** fully checked — which is what makes it a real lock rather than decoration.

### 1.6 Where the sources disagree

Six disagreements are decision-shaped and stated in full in §6 — `OPEN-1`, `OPEN-3`, `OPEN-4`, `OPEN-5`,
`OPEN-12`, `OPEN-14`. A seventh is cosmetic: §5/D001 name "`V1__init.sql`" for what is actually
`V1__foundation.sql`.
...
**What the double composite FK is for.** `(project_id, requirement_id) -> requirement(project_id, id)`
proves the requirement exists and is in this project; the second FK is the anti-divergence device, since
with `source_requirement_id` as its third referenced column the database refuses any relation row that
disagrees with the document. §2.3:
...
`PRD.md` P3: "需求附件只在所属需求的 AI 场景可见；跨需求共享必须显式提升为项目知识". For the code: promotion is an
**INSERT**, never an `UPDATE` — no path flips `source_type` or nulls `source_requirement_id`, and the
relation FK would block it (`23503`) if one did. Afterwards two documents share the same `text`: the
untouched attachment, and a new public one with `source_requirement_id IS NULL`; the relation row
survives verbatim. The new document needs its own `knowledge_chunk` rows, since chunks are keyed by
`document_id` — re-embed or vector-copy is unstated (`OPEN-6`), and it drives cost and test determinism.
Testable invariant: promotion leaves the source byte-identical, and
`count(*) FROM requirement_attachment WHERE document_id = :orig` is still 1.
...
transient classes only (429 / 5xx / network / timeout), count exactly 1. A retry in `knowledge`, around
the database, or on a non-429 4xx has no authorization.
...
transformation, not refusal. The only mentions in `docs/v2/` are §4.3 (Prompt 安全, the whole section):
"…**全部是不可信数据**，不得改变 system/task 指令；发送前做敏感信息脱敏与预算裁剪。"; the matrix row
"`ai/PromptSanitizer` | 敏感信息脱敏、Unicode 截断 | KEEP | 纯函数；继续补注入与密钥格式测试"; §8's
"不可信标记 + source 白名单 + 输出回查"; and the plan's "安全测试必须覆盖 … Prompt injection."
...
Written by `ai` for **both** `chat` and `embed` (§1.2: "chat/embed 协议与**调用记录**"), never surfaced
as a page (§6: "AI Logs 均**不做**一级页面"), and consumed by Phase 8 evaluation ("可复现评测：… Token、
耗时"). `INFERENCE` forced by "故障定位": the row must be written on **failure** too and outside the
caller's transaction, or a rolled-back Review erases why it failed. `OPEN-10`.
...
| The gateway itself | HTTP shape, auth header, timeout, exactly-one retry, error classification, `ai_call_log` contents | a **real local socket** with canned responses |

For the upper tier, `@TestBean` (spring-test 7.0.8, verified present) swaps the `AiGateway` bean via a
static factory method — no Mockito DSL, the fake is ordinary Java; `@MockitoBean` is the alternative when
a test wants call verification. The rest of the context is untouched, so the real PostgreSQL, Flyway,
security chain and error contract still run — exactly what the dimension-mismatch and attachment-scoping
tests need: a real database and only a fake provider.
...
`429` / `503` / malformed JSON, driving `AiFailurePolicy`; sleep past the timeout, proving it actually
fires; **count requests**, the only way to prove "exactly one retry, transient classes only" — a
permanent `400` must leave the counter at 1; and assert the `Authorization` header carried the dummy
key, proving auth wiring without any real key existing. Wire it with `@DynamicPropertySource` setting
`ai.base-url = http://127.0.0.1:<port>`, the mechanism `PostgresTestBase:33-38` already uses; bind port
`0` so CI never collides.
...
"*mockwebserver*"` → no output). Either would be a new dependency, and `quality-guidelines.md` requires
a concrete failure a new gate would have caught; there is none, since the JDK server covers every listed
behavior. Also verified present: `spring-boot-starter-tomcat` (so `RANDOM_PORT` is available if a real
port is ever wanted), `RestClient` in `spring-web` 7.0.8, `@MockitoBean` and `@TestBean`.

### 4.3 What a deterministic fake embedding looks like
...
static float[] fake(String text, int dim) {      // dim = configured profile dimension
    ...
    for (int i = 0; i < dim; i++) {              // xorshift; L2-normalised below
        h ^= (h << 13); h ^= (h >>> 7); h ^= (h << 17);
...
**The caveat that must not be buried.** A hash fake gives determinism but **not semantic similarity** —
"登录接口" and "authentication endpoint" land nowhere near each other. Tests against it can honestly
assert **isolation, filtering, TopK size, ordering stability and dimension handling**, plus an exact hit
when the query is byte-identical to a chunk (distance 0, rank 1). They cannot assert retrieval
*quality*; that belongs to the Phase 6 development set and the Phase 8 holdout with a real provider,
where the plan already puts it. A Phase 4 test claiming semantic recall against a hash fake would be
exactly the false green `database-guidelines.md` bans elsewhere. If a test needs controlled
neighbourhood structure, let the fixture text carry its own coordinates rather than pretending the hash
is a language model.

### 4.4 The one thing this does not solve
...
sides: 5 MB + 1 byte rejected, 5 MB exactly accepted. **Layering trap:** if `spring.servlet.multipart.max-file-size` is at or below the same limit, Spring
raises `MaxUploadSizeExceededException` in the filter chain and the validator never runs — and that must
still produce the one `ApiError` body, not a Spring default page. Test the **body**, not just the status,
and decide deliberately which of the two rejects.

**5.5 维度不匹配.** Source: D001 "应用写入时显式校验维度"; §5 "应用层写入时校验向量维度与当前 Profile 一致，
不一致显式失败." (1) With the `dim + 1` fake, ingestion fails explicitly — document ends `FAILED`, **zero**
`knowledge_chunk` rows written, failure visible to an admin (§6 "管理员只需看到文档状态与失败原因", where
`OPEN-3` bites since no column can hold the reason). (2) **The honest one**: prove the database does *not*
protect you — insert a wrong-dimension vector via `JdbcTemplate` and assert it **succeeds**, because
`embedding` is typmod-less (D001). That test is what justifies the application-layer check existing.
(3) `knowledge_chunk.dimension` equals the vector's actual length for every written row.
...
scope, both → `23514`. Retrieval boundary (the P3/D005 test): in one project seed requirement R with attachment Da, R' with Db,
and public document Dp; search scoped to R returns Da and Dp and **never** Db, then the mirror for R'.
...
`PRD.md` §4, yet present in **none** of the 16 tables, so `INFERENCE`: response-only, never persisted,
with only `ai_call_log` recording that the call happened (`OPEN-13`).
...
| OPEN-8 | Enum values: `source_type`'s public value, `ai_call_log.status`, `ai_call_log.use_case`. | §2.4 requires a DB `CHECK` listing them; frozen at migration time |
...
| OPEN-13 | Is Implementation Guidance really never persisted? No table holds it. | A reload re-costs an AI call; limits what a demo can show |
...
- **Legacy source is not in this repository** — the matrix points at read-only `LinYsssss/reposage`, so
  the two KEEP classes this batch depends on, `PromptSanitizer` and `KnowledgeUploadValidator`, could not
  be read. §3.3 and §5.2–5.4 derive from the matrix's one-line descriptions plus the V2 documents;
  fetching those two files is worth doing before design.
...
- **Read, not measured.** Nothing here was run against a database. Whether Hibernate 7.4.1 accepts a
  scalar-only mapping for `ai_call_log`'s four ids should be verified the way D013.1 was — by starting the
  context, early. The classpath facts in §4.2 *were* executed and are reliable.
